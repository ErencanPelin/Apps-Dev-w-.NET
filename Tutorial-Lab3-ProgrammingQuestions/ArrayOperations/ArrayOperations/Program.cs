...
namespace Week4LabProgramQuestion
{
    class ArrayDB
    {
        ...
        {
            double[] val = new double[numberOfElement];
            for (int loopVar = 0; loopVar < numberOfElement; loopVar++)
            {
                val[loopVar] = Math.Pow(userData[loopVar] - meanValue, 2);
            }
            double sMean = 0;
            for (int loopVar = 0; loopVar < numberOfElement; loopVar++)
            {
            ...
            varianceValue = sMean / (numberOfElement - 1);
            return (varianceValue);
        }
        // Method to calculate Standard Deviation
        public double CalculateSD()
        {
            sdValue = Math.Sqrt(CalulateVariance());
            return (sdValue);
        }        
    }
